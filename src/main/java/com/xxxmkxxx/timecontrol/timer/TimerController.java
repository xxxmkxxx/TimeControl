...
public record TimerController(Timers timers, GameTime GAME_TIME) {
    public AbstractTimer startTimer(String name, int seconds, Task task, boolean execConstantly) {
        AbstractTimer timer = new SimpleTimer(GAME_TIME.getCurrentTick(), seconds, task, execConstantly, new StartTimerState(task));
